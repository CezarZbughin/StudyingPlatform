...
DELIMITER $$
USE `StudyingPlatform`$$
CREATE PROCEDURE try_(given_id INT)
BEGIN
DECLARE vrole ENUM('STUDENT', 'PROFESSOR') DEFAULT null;
SET vrole = (SELECT role FROM user WHERE id = given_id);
IF vrole = 'STUDENT' THEN
...
-- -----------------------------------------------------
-- procedure try_sign_up
-- -----------------------------------------------------

DELIMITER $$
USE `StudyingPlatform`$$
CREATE PROCEDURE `try_sign_up`(pusername VARCHAR(32),ppassword VARCHAR(32))
BEGIN
DECLARE is_succesful BOOLEAN;
DECLARE username_found BOOLEAN;
DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        ROLLBACK;  -- rollback any changes made in the transaction
        RESIGNAL;  -- raise again the sql exception to the caller
    END;
START TRANSACTION;
	SET is_succesful = (SELECT EXISTS(SELECT id from user WHERE username=pusername AND password=ppassword));
	SET username_found = (SELECT EXISTS(SELECT id from user WHERE username=pusername));
    SELECT is_succesful, username_found, id from user where username=pusername;
COMMIT;
END$$
